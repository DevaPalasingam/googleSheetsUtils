...
function syncCalendarK1() {
    ...
    if (curDateValue != null || isValidDate(dateOnCell)) {
      ...
      if (!isValidDate(eventStart)) {
        if (
          ...
          eventEnd = new Date(
            curDateValue.getUTCFullYear(),
            curDateValue.getUTCMonth(),
            curDateValue.getUTCDate() + 1,
            curDateValue.getHours(),
            0
          );
          toCreate.push({
            name: eventName,
      ...
      if (eventEnd.getTime() < eventStart.getTime()) {
        //hack fix for when the end time is 12AM or something. Add 24 hours to move it to next day.
  ...
  //  Logger.log(".................");
  while (
    firstDateValue.getTime() <= curDateValue.getTime() &&
    toCreate.length > 0
  ) {
    ...
    for (var e in events) {
      ...
      for (var item in toCreate) {
        ...
        if (
          toCreate[item]["date"].getTime() == firstDateValue.getTime() &&
          toCreate[item]["name"] == events[e].getTitle() &&
          events[e].getStartTime().valueOf() ==
            toCreate[item]["start"].valueOf() &&
      ...
      if (
        matching == null &&
        (events[e].getStartTime().valueOf() >=
          firstDateValue.getTime().valueOf() ||
          events[e].isAllDayEvent())
      ) {
        //calendar event not found, deleting
        toDelete[events[e].getId()] = events[e];
      } else {
        //calendar event found, don't create (i.e. remove from create list)
